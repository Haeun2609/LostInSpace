      ...
      body.shapes.forEach(function (shape) {
        ...
        switch (shape.type) {
          ...
          case CANNON.Shape.types.HEIGHTFIELD:
            ...
            var cols = [{
              stop: 0,
              color: new THREE.Color('#5a3b72')
            }, {
              stop: .25,
              color: new THREE.Color('#3f2d76')
            }, {
              stop: .5,
              color: new THREE.Color('#081664')
            }, {
              stop: .75,
              color: new THREE.Color('#04447b')
            }, {
              stop: 1,
              color: new THREE.Color('#065c6c8')
            }];
...
Promise.all([
  img2matrix.fromUrl('https://upload.wikimedia.org/wikipedia/commons/5/57/Heightmap.png', sizeX, sizeY, minHeight, maxHeight)(),
    ...
    check = function () {
        ...
        var raycaster2 = new THREE.Raycaster(starbit.position, new THREE.Vector3(0, -1, 0));
        var intersects2 = raycaster2.intersectObject(terrainBody.threemesh.children[0]);

        //position objects ontop of the terrain
        starbit.position.y = intersects2 && intersects2[0] ? intersects2[0].point.y + .5 : 30;
        starLight.position.y = starbit.position.y + 50;
        starLight.position.x = starbit.position.x + 5
        starLight.position.z = starbit.position.z;
    }
})
...
//STARTBITS===============================================
var geom = new THREE.IcosahedronGeometry(0.15,0);
geom.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 0.075, 0));
var mat = new THREE.MeshNormalMaterial()
var starbit = new THREE.Mesh(geom,mat);
starbit.position.x = randnum(0,20);
starbit.position.z = randnum(0,20);
scene.add(starbit)
var starLight = new THREE.PointLight(new THREE.Color('blue'),1.5,5);
starbit.add(starLight)
//LIGHTS==================================================
